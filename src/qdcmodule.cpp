    ...
    handleChef(handleChef_)
{
    ...
    rightShiftBitForChannelIndex = 0;
    nBitForChannelIndex = 0;
    stepForRegisterChannelIndex = 0;
    nChannels = 0;


}

QString QDCModule::setBaseAddress(int baseAddress_)
{
...
QString QDCModule::getModuleType()
{
    u_int16_t value16;

    // check fixed code of the board first
    CVErrorCodes res = CAENVME_ReadCycle(handleChef, baseAddress + 0x8032, &value16, cvA32_U_DATA, cvD16);

    if (res != cvSuccess || ((value16 & 0xff) != (u_int8_t)19 && (value16 & 0xff) != (u_int8_t)227)) {
        qDebug() << "Wrong board";
        return "UNKNOWN";
    }
    else if ((value16 & 0xff) == 19){
